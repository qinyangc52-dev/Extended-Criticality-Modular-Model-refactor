...
void network::sitipattern(void)		
  {
   fprintf(stderr,"[network] selecting module paths: topo=%s, P=%i, S=%i, G=%i\n",topo,P,S,G);
   fflush(stderr);
   if (strcmp(topo,"random")==0)
      ...
      for (int p=0;p<P;p++)
        {
         ...
         fprintf(stderr,"[network] pattern path %i/%i selected (random)\n",p+1,P);
         fflush(stderr);
        }
     }
#if 1											
   else if (strcmp(topo,"tract1")==0 || strcmp(topo,"tract2")==0)
     {
      ...
      for (int p=0;p<P;p++)
        {
         fprintf(stderr,"[network] pattern path %i/%i: searching closed walk\n",p+1,P);
         fflush(stderr);
         int ntry=0;
         int maxtry=10000;
         while (1)
           {
            ...
            if (ntry%1000==0)
              {
               fprintf(stderr,"[network] pattern path %i/%i: %i closed-walk attempts\n",p+1,P,ntry);
               fflush(stderr);
              }
            if (ntry>maxtry) debug(9,"p=%i... non riesco a trovare cammini chiusi... aumentare maxtry?\n",p);
           }
         fprintf(stderr,"[network] pattern path %i/%i selected after %i attempts\n",p+1,P,ntry+1);
         fflush(stderr);
         int i=G;
         while (i<S)
           {
...
void network::hmatrix(void)
  {
   fprintf(stderr,"[network] computing pattern sizes H\n");
   fflush(stderr);
   for (int p=0;p<P;p++)
     {
      H[p]=0;
   ...
   for (int p=0;p<P;p++)
     {
      ...
      fprintf(stderr,"[network] neurons for pattern %i/%i selected (H=%i)\n",p+1,P,h);
      fflush(stderr);
     }
   free(stack);
  }
...
void network::creaposix(void)
  {
   fprintf(stderr,"[network] creating pattern position indexes\n");
   fflush(stderr);
...
void network::fasi(void)
  {
   fprintf(stderr,"[network] assigning phases: sort=%i\n",sort);
   fflush(stderr);
   if (sort==0)
     {
      for (int p=0;p<P;p++)
        {
         fprintf(stderr,"[network] phases for pattern %i/%i: H=%i\n",p+1,P,H[p]);
         fflush(stderr);
...
void network::connessioni(void)
  {
   fprintf(stderr,"[network] building STDP connectivity: P=%i, N=%i\n",P,N);
   fflush(stderr);
   double T=1000/f;		
   int nmax=ceil(286/T);
   for (int p=0;p<P;p++)
     {
      fprintf(stderr,"[network] connectivity pattern %i/%i started (H=%i)\n",p+1,P,H[p]);
      fflush(stderr);
      ...
      fprintf(stderr,"[network] connectivity pattern %i/%i done\n",p+1,P);
      fflush(stderr);
     }
  }
...
network::network(void)
  {
   ...
   fprintf(stderr,"[network] initializing network parameters\n");
   fflush(stderr);
   if (strcmp(topo,"random")==0)
     {
   ...
   fprintf(stderr,"[network] allocated arrays: N=%i, dense J memory=%.3f GB\n",N,8e-9*N*(double)N);
   fflush(stderr);
   H=ivector(0,P-1);
   ...
   if (stream)
   ...
   else
     {
      ...
      fprintf(stderr,"[network] generating patterns and connectivity from seed\n");
      fflush(stderr);
      sitipattern();
      hmatrix();
      neuronipattern();
      fasi();
      connessioni();
      fprintf(stderr,"[network] connectivity generated; writing module summary\n");
      fflush(stderr);
      FILE *stream3=stdout;
   ...
   creaposix();
   readcues();
   fprintf(stderr,"[network] network initialization complete\n");
   fflush(stderr);
  }
#endif
#endif
